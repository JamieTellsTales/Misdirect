...
func _handle_click(pos: Vector2) -> void:
	# Power up (radio — one at a time, click again to deselect)
	for i in POWER_UPS.size():
		var row_y: float = power_up_start_y + i * ROW_HEIGHT
		var rect := Rect2(COL_RADIO - 16, row_y - 20, ARENA_WIDTH - COL_RADIO - 60, 40)
		if rect.has_point(pos):
			GameConfig.set_power_up(POWER_UPS[i]["id"])
			return
...
func _draw_power_ups() -> void:
	...
	modifier_start_y = power_up_start_y + POWER_UPS.size() * ROW_HEIGHT + SECTION_GAP + 40.0
...
func _draw_radio(center: Vector2, selected: bool, hovered: bool) -> void:
	var ring_col := Color(0.7, 0.7, 0.85, 1.0) if hovered else Color(0.45, 0.45, 0.6, 1.0)
...
func _draw_option_text(font: Font, label: String, desc: String,
	...
	var label_col := Color(1.0, 1.0, 1.0, 1.0) if active else (
		Color(0.85, 0.85, 0.95, 1.0) if hovered else Color(0.6, 0.6, 0.72, 1.0)
	)
